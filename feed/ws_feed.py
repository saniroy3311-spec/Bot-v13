    ...
    async def _process_ws_candle(self, data: dict) -> None:
        ...
        if current_boundary > self._last_candle_boundary:
            ...
            if self.trail_monitor is not None:
                ...
                # Use Delta as an exit trigger source only when explicitly
                # enabled. Previously push_delta_tick() ran unconditionally,
                # so a simultaneous Binance exit feed produced two competing
                # trigger streams despite TRAIL_EXIT_FROM_DELTA_WS=false.
                if TRAIL_EXIT_FROM_DELTA_WS:
                    loop = asyncio.get_running_loop()
