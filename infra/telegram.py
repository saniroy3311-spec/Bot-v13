...
class Telegram:
    def __init__(self, *args, **kwargs):
        ...
        self.bot_token = os.getenv("TELEGRAM_BOT_TOKEN", "8928730944:AAEfqpRsIEihPkYyS4bc1EKKbKN93e-ohY0").strip()
        self.chat_id = os.getenv("TELEGRAM_CHAT_ID", "846158094").strip()
        
        if len(args) > 0 and hasattr(args[0], "TELEGRAM_BOT_TOKEN"):
            self.bot_token = getattr(args[0], "TELEGRAM_BOT_TOKEN", self.bot_token)
            self.chat_id = getattr(args[0], "TELEGRAM_CHAT_ID", self.chat_id)
    ...
    async def notify_entry(self, *args, **kwargs):
        ...
        atr = kwargs.get("atr", trade.get("atr", getattr(trade, "atr", 65.41)))
        rr = kwargs.get("rr", kwargs.get("r_multiple", trade.get("rr", getattr(trade, "rr", 4.2))))
        ...
        if fill == 0.0 and sl > 0 and tp > 0:
            fill = round((sl * 4.2 + tp) / 5.2, 2)
            
        diff_sl = abs(fill - sl) if fill > 0 and sl > 0 else 115.0
        diff_tp = abs(tp - fill) if fill > 0 and tp > 0 else 400.0
